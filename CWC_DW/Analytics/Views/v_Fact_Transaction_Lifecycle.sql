...
CREATE VIEW [Analytics].[v_Fact_Transaction_Lifecycle] AS
...
,Backorders AS
	(
		SELECT
			...
			,[Customer Key] = b.Sales_Header_Customer_Key
			,l.[State]
			,[Zip] = l.[Zip_Code]
			,[Net Quantity] = NULL
			,[Net Amount] = NULL
...
,Sales AS
	(
		SELECT --TOP 5000 
			...
			,[Customer Key] = s.Sales_Header_Customer_Key
			,l.[State]
			,[Zip] = l.[Zip_Code]
			,s.Invent_Trans_ID
		FROM [CWC_DW].[Fact].[Sales] (NOLOCK) s
...
,Returns AS
	(
		SELECT --TOP 500
		    ...
		    ,[Delivery Mode] = shp.[Sales_Header_Delivery_Mode]
			,[Customer Key] = r.Sales_Header_Customer_Key
			,l.[State]
			,[Zip] = l.[Zip_Code]
			,s.Invent_Trans_ID
			,[Net Quantity] = NULL
...
,Shipments AS
	...
	,[Customer Key] = i.Invoice_Customer_Key
	,s.[State]
	,[Zip] = s.[Zip]
	,s.Invent_Trans_ID
	,[Net Quantity] = NULL
