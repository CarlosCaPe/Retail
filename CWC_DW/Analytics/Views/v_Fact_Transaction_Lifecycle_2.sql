...
CREATE VIEW [Analytics].[v_Fact_Transaction_Lifecycle_2] AS
...
,Backorders AS
	(
		SELECT
			...
			,[Transaction Lifecycle Date] = b.Sales_Line_Created_Date_EST--CAST(sd.Calendar_Date AS DATE)
			...
			,[Price] = CASE WHEN b.Sales_Line_Price < ISNULL(NULLIF(b.Sales_Line_Cost_Price,0),p.[PO_Weighted_Average_Unit_Cost]) THEN  ISNULL(NULLIF(tp.Price,0),NULLIF(b.Sales_Line_Price,0)) ELSE ISNULL(NULLIF(b.Sales_Line_Price,0),NULLIF(tp.Price,0)) END
			,[Charge] = NULL
...
,Sales AS
	(
		SELECT --TOP 5000 
			 ...
			 [Transaction Lifecycle Date] = CAST(s.Sales_Line_Created_Date_EST AS DATE)
			,[Transaction Date] = CAST(s.Sales_Line_Created_Date_EST AS DATE)
			...
			,[Price] = CASE WHEN s.Sales_Line_Price < ISNULL(NULLIF(s.Sales_Line_Cost_Price,0),p.[PO_Weighted_Average_Unit_Cost]) THEN  ISNULL(NULLIF(tp.Price,0),NULLIF(s.Sales_Line_Price,0)) ELSE ISNULL(NULLIF(s.Sales_Line_Price,0),NULLIF(tp.Price,0)) END
			,[Charge] = NULL
			,[B2B] = CASE WHEN s.Sales_Header_Customer_Key IN (SELECT Customer_Key FROM B2B) THEN 'Yes' ELSE 'No' END
			,[Backorder Fill] = NULL --CASE WHEN b.Sales_Key IS NULL THEN 'No' ELSE 'Yes' END --CASE WHEN sb.[is_Backorder_Fill] = 1 THEN 'Yes' ELSE 'No' END
		...
		WHERE s.Sales_Line_Created_Date_EST >= '1/1/2019' 
...
,Returns AS
	(
		...
		WHERE r.[Sales_Line_Created_Date_EST] >= '1/1/2019' 
...
,Shipments AS
	...
	,[Order Unique] = DENSE_RANK() OVER(ORDER BY i.Invoice_Number)
	--,i.Invoice_Number
