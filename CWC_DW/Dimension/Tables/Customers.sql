...
GO
 CREATE TRIGGER [Dimension].[tr_Customers_Audit] ON [Dimension].[Customers]
AFTER INSERT
	,UPDATE
	,DELETE
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			IF EXISTS (
					SELECT NULL
					FROM inserted i
					INNER JOIN deleted d ON i.Customer_Key = d.Customer_Key
	...
	EXEC [Administration].[usp_Audit_Table_Insert] '[Dimension].[Customers]'
		,@Operation
		,@RecordCount
END