...
GO
 CREATE TRIGGER [Fact].[tr_Invoices_Audit] ON [Fact].Invoices
AFTER INSERT
	,UPDATE
	,DELETE
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			IF EXISTS (
					SELECT NULL
					FROM inserted i
					INNER JOIN deleted d ON i.Invoice_Key = d.Invoice_Key
	...
	EXEC [Administration].[usp_Audit_Table_Insert] '[Fact].[Invoices]'
		,@Operation
		,@RecordCount
END