...
GO
 CREATE TRIGGER [Fact].[tr_Purchase_Orders_Audit] ON [Fact].Purchase_Orders
AFTER INSERT
	,UPDATE
	,DELETE
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			IF EXISTS (
					SELECT NULL
					FROM inserted i
					INNER JOIN deleted d ON i.Purchase_Order_Key = d.Purchase_Order_Key
	...
	EXEC [Administration].[usp_Audit_Table_Insert] '[Fact].[Purchase_Orders]'
		,@Operation
		,@RecordCount
END