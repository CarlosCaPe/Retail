...
CREATE PROC [Import].[usp_Dimension_Catalog_Promotions_Merge]
AS
SET NOCOUNT ON
...
BEGIN TRY
	...
	AS (
		SELECT *
		FROM [CWC_DW].[Dimension].[Catalog_Promotions]
		)
	MERGE INTO tgt
	USING src
	...
	WHEN MATCHED
		AND ISNULL(tgt.[Catalog_Key], '') <> ISNULL(src.[Catalog_Key], '')
		...
		THEN
			UPDATE
			SET [Catalog_Key] = src.[Catalog_Key]
