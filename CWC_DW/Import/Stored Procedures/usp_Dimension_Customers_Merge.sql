...
SELECT DISTINCT
	  ...
	  --,[Created_Date_Key] = cd.Date_Key
	  --,[Modified_Date_Key] = cd.Date_Key
	  ,[Employee_Responsible_Number] = NULLIF([EMPLOYEERESPONSIBLENUMBER],'')
	  ,[Customer_Group] = CASE WHEN [CUSTOMERGROUPID] = 'CCCUST' THEN 'Contact Center' 
							   WHEN [CUSTOMERGROUPID] = 'CUST' THEN 'Ecommerce'
							   WHEN [CUSTOMERGROUPID] = 'Retail' THEN 'Retail'
							   WHEN [CUSTOMERGROUPID] = 'Emp' THEN 'Employee'
							   WHEN [CUSTOMERGROUPID] = 'Jobber' THEN 'Jobber'
      ...
      --,[Person_Middle_Name] = NULLIF([PERSONMIDDLENAME],'')
      --,[Person_Last_Name_Prefix] = NULLIF([PERSONLASTNAMEPREFIX],'')
      ...
      --,[Address_Location_ID] = NULLIF([ADDRESSLOCATIONID],'')
      ...
      ,[Payment_Terms] = NULLIF(CASE [PAYMENTTERMS] WHEN 'N00' THEN 'Due Upon Receipt'
													WHEN 'N30' THEN 'Net 30 Days'
													WHEN 'Cash' THEN 'Cash'
													WHEN 'CreditCard' THEN 'Credit Card'
													ELSE [PAYMENTTERMS] END,'')
      ,[Payment_Method] = NULLIF(CASE [PAYMENTMETHOD] WHEN 'CreditCard' THEN 'Credit Card' ELSE [PAYMENTMETHOD] END,'')
      ,[Delivery_Mode] = NULLIF(CASE [DELIVERYMODE] WHEN 'STANDARD' THEN 'Standard Ground'
      ...
      ,[Email_Contact_Pref_ID] = NULLIF(CASE [ITSCUSTEMAILCONTACTPREFID] WHEN 'DoNotEmail' THEN 'Do Not Email' ELSE [ITSCUSTEMAILCONTACTPREFID] END,'')
      ,[Phone_Contact_Pref_ID] = NULLIF(CASE [ITSCUSTTELEPHONECONTACTPREFID] WHEN 'DoNotPhone' THEN 'Do Not Phone' ELSE [ITSCUSTTELEPHONECONTACTPREFID] END,'')
      ,[Direct_Mail_Contact_Pref_ID] = NULLIF(CASE [ITSCUSTDIRECTMAILCONTACTPREFID] WHEN 'DoNotMail' THEN 'Do Not Mail' ELSE [ITSCUSTDIRECTMAILCONTACTPREFID] END,'')
      ,[Rental_Contact_Pref_ID] = NULLIF(CASE [ITSCUSTRENTALCONTACTPREFID] WHEN 'DoNotRent' THEN 'Do Not Rent' ELSE [ITSCUSTRENTALCONTACTPREFID] END,'')
	  ...
	  ,[is_Business_Address] = CAST((CASE WHEN [ADDRESSLOCATIONROLES] LIKE '%Business%' THEN 1 ELSE 0 END) AS BIT)
	  ,[is_Delivery_Address] = CAST((CASE WHEN [ADDRESSLOCATIONROLES] LIKE '%Delivery%' THEN 1 ELSE 0 END) AS BIT)
	  ,[Seq] = ROW_NUMBER() OVER(PARTITION BY [CUSTOMERACCOUNT] ORDER BY [ITSCUSTMODIFIEDDATETIME] DESC, [ITSCUSTMODIFIEDDATETIME] DESC)
...
-- INTO CWC_DW.Dimension.Customers
 FROM [AX_PRODUCTION].[dbo].[CustCustomerV3Staging] (NOLOCK) c
  LEFT JOIN [CWC_DW].[Dimension].[Employees] e ON c.[EMPLOYEERESPONSIBLENUMBER] = e.[Personnel_Number] AND CAST([ITSCUSTMODIFIEDDATETIME] AS DATE) BETWEEN e.Start_Date AND ISNULL(e.End_Date,CAST(SYSUTCDATETIME() AS DATE))

  --WHERE CUSTOMERACCOUNT = 'EC06426085'
  /*
  SELECT  [ITSCUSTEMAILCONTACTPREFID],COUNT(*) 
  FROM [AX_PRODUCTION].[dbo].[CustCustomerV3Staging] (NOLOCK)
  GROUP BY [ITSCUSTEMAILCONTACTPREFID]
  */
  )
  ,src AS(SELECT * FROM src_seq WHERE seq = 1)
--  SELECT * FROM src WHERE Customer_Account IN ('000007_045'
--,'DC00483040'
--,'DC00145601'
--,'EC03584275'
--,'DC00483468'
--,'DC00483471')
  ...
  ,tgt AS (SELECT * FROM [CWC_DW].[Dimension].[Customers] (NOLOCK))

  MERGE INTO tgt USING src
	ON tgt.[Customer_Account] = src.[Customer_Account]
		WHEN NOT MATCHED THEN
			INSERT([Customer_Account],[Employee_Key],[Employee_Responsible_Number],[Customer_Group],[Organization_Name],[Name_Alias],[Known_As],[Person_First_Name],[Person_Last_Name],[Full_Primary_Address]
				  ,[Address_Description],[Address_Zip_Code],[Address_City],[Address_County],[Address_State],[Address_Country_Region_ID],[Address_Valid_From],[Address_Valid_To],[Primary_Contact_Email]
				  ,[Primary_Contact_Phone],[Sales_Currency_Code],[Payment_Terms],[Payment_Method],[Delivery_Mode],[Receipt_Email],[Delivery_Address_Description],[Delivery_Address_Zip_Code],[Delivery_Address_City]
				  ,[Delivery_Address_Country_Region_ID],[Delivery_Address_County],[Delivery_Address_State],[Delivery_Address_Street],[Delivery_Address_Street_Number],[Delivery_Address_Valid_From],[Delivery_Address_Valid_To]
				  ,[Email_Contact_Pref_ID],[Phone_Contact_Pref_ID],[Direct_Mail_Contact_Pref_ID],[Rental_Contact_Pref_ID],[Created_Date_Time],[Modified_Date_Time],[On_Hold_Status],[is_Business_Address]
		          ,[is_Delivery_Address],[is_Removed_From_Source],[is_Excluded],[ETL_Created_Date],[ETL_Modified_Date],[ETL_Modified_Count])
			VALUES(src.[Customer_Account],src.[Employee_Key],src.[Employee_Responsible_Number],src.[Customer_Group],src.[Organization_Name],src.[Name_Alias],src.[Known_As],src.[Person_First_Name],src.[Person_Last_Name],src.[Full_Primary_Address]
				  ,src.[Address_Description],src.[Address_Zip_Code],src.[Address_City],src.[Address_County],src.[Address_State],src.[Address_Country_Region_ID],src.[Address_Valid_From],src.[Address_Valid_To],src.[Primary_Contact_Email]
				  ,src.[Primary_Contact_Phone],src.[Sales_Currency_Code],src.[Payment_Terms],src.[Payment_Method],src.[Delivery_Mode],src.[Receipt_Email],src.[Delivery_Address_Description],src.[Delivery_Address_Zip_Code],src.[Delivery_Address_City]
				  ,src.[Delivery_Address_Country_Region_ID],src.[Delivery_Address_County],src.[Delivery_Address_State],src.[Delivery_Address_Street],src.[Delivery_Address_Street_Number],src.[Delivery_Address_Valid_From],src.[Delivery_Address_Valid_To]
				  ,src.[Email_Contact_Pref_ID],src.[Phone_Contact_Pref_ID],src.[Direct_Mail_Contact_Pref_ID],src.[Rental_Contact_Pref_ID],src.[Created_Date_Time],src.[Modified_Date_Time],src.[On_Hold_Status],src.[is_Business_Address]
		          ,src.[is_Delivery_Address],src.[is_Removed_From_Source],src.[is_Excluded],src.[ETL_Created_Date],src.[ETL_Modified_Date],src.[ETL_Modified_Count])
			WHEN MATCHED AND			
				      ISNULL(tgt.[Employee_Key],'') <> ISNULL(src.[Employee_Key],'')
