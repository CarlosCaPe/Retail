...
BEGIN TRY
	...
	(
	SELECT
		   [Date_Key] = REPLACE(CAST(CAST(d.[Calendar_Date] as [Date]) AS VARCHAR(10)),'-','')
		  ,[Calendar_Date]
		  ,[Calendar_Month]
		  ,[Calendar_Day]
		  ,[Calendar_Year]
	  ...
	  ,tgt AS (SELECT * FROM [CWC_DW].[Dimension].[Dates])
  
	  MERGE INTO tgt USING src
		 ON tgt.[Date_Key] = src.[Date_Key]
	  WHEN NOT MATCHED THEN
	  INSERT
		 ([Date_Key],[Calendar_Date],[Calendar_Month],[Calendar_Day],[Calendar_Year],[Calendar_Quarter],[Calendar_Quarter_Name],[Calendar_Quarter_Year_Name],[Day_Name],[Day_of_Week]
		 ,[Day_of_Week_in_Month],[Day_of_Week_in_Year],[Day_of_Week_in_Quarter],[Day_of_Quarter],[Day_of_Year],[Week_of_Month],[Week_of_Quarter],[Week_of_Year],[Month_Name],[First_Date_of_Week]
		 ,[Last_Date_of_Week],[First_Date_of_Month],[Last_Date_of_Month],[First_Date_of_Quarter],[Last_Date_of_Quarter],[First_Date_of_Year],[Last_Date_of_Year],[is_Holiday],[is_Holiday_Season]
		 ,[Holiday_Name],[Holiday_Season_Name],[is_Weekday],[is_Business_Day],[Previous_Business_Day],[Next_Business_Day],[is_Leap_Year],[Days_in_Month],[Fiscal_Day_of_Year],[Fiscal_Week_of_Year]
		 ,[Fiscal_Month],[Fiscal_Quarter],[Fiscal_Quarter_Name],[Fiscal_Quarter_Year_Name],[Fiscal_Year],[Fiscal_Year_Name],[Fiscal_Month_Year],[Fiscal_MMYYYY],[Fiscal_First_Day_of_Month]
		 ,[Fiscal_Last_Day_of_Month],[Fiscal_First_Day_of_Quarter],[Fiscal_Last_Day_of_Quarter],[Fiscal_First_Day_of_Year],[Fiscal_Last_Day_of_Year],[is_Removed_From_Source],[is_Excluded],[ETL_Created_Date]
		 ,[ETL_Modified_Date],[ETL_Modified_Count])
	  VALUES
		 (src.[Date_Key],src.[Calendar_Date],src.[Calendar_Month],src.[Calendar_Day],src.[Calendar_Year],src.[Calendar_Quarter],src.[Calendar_Quarter_Name],src.[Calendar_Quarter_Year_Name],src.[Day_Name],src.[Day_of_Week]
		 ,src.[Day_of_Week_in_Month],src.[Day_of_Week_in_Year],src.[Day_of_Week_in_Quarter],src.[Day_of_Quarter],src.[Day_of_Year],src.[Week_of_Month],src.[Week_of_Quarter],src.[Week_of_Year],src.[Month_Name],src.[First_Date_of_Week]
		 ,src.[Last_Date_of_Week],src.[First_Date_of_Month],src.[Last_Date_of_Month],src.[First_Date_of_Quarter],src.[Last_Date_of_Quarter],src.[First_Date_of_Year],src.[Last_Date_of_Year],src.[is_Holiday],src.[is_Holiday_Season]
		 ,src.[Holiday_Name],src.[Holiday_Season_Name],src.[is_Weekday],src.[is_Business_Day],src.[Previous_Business_Day],src.[Next_Business_Day],src.[is_Leap_Year],src.[Days_in_Month],src.[Fiscal_Day_of_Year],src.[Fiscal_Week_of_Year]
		 ,src.[Fiscal_Month],src.[Fiscal_Quarter],src.[Fiscal_Quarter_Name],src.[Fiscal_Quarter_Year_Name],src.[Fiscal_Year],src.[Fiscal_Year_Name],src.[Fiscal_Month_Year],src.[Fiscal_MMYYYY],src.[Fiscal_First_Day_of_Month]
		 ,src.[Fiscal_Last_Day_of_Month],src.[Fiscal_First_Day_of_Quarter],src.[Fiscal_Last_Day_of_Quarter],src.[Fiscal_First_Day_of_Year],src.[Fiscal_Last_Day_of_Year],src.[is_Removed_From_Source],src.[is_Excluded],src.[ETL_Created_Date]
		 ,src.[ETL_Modified_Date],src.[ETL_Modified_Count])
	  WHEN MATCHED AND
			ISNULL(tgt.[Calendar_Date],'') <> ISNULL(src.[Calendar_Date],'')
