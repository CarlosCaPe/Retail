...
BEGIN TRY
	...
	WITH emp
	AS (
		SELECT Personnel_Number = e.PersonnelNumber
			,User_ID = u.Userid
			,Employee_Full_Name = e.Name
			,Username = u.Username
			,License_Source = CASE 
				WHEN left(PERSONNELNUMBER, 2) = 'CC'
					THEN 'Global Response'
				WHEN left(PERSONNELNUMBER, 2) = 'CU'
					THEN 'ContactUS'
				WHEN isnumeric(PERSONNELNUMBER) = 1
					THEN 'CWD Corporate'
				WHEN (
						left(PERSONNELNUMBER, 3) IN (
							'CWC'
							,'CWD'
							,'CWS'
							)
						AND EMPLOYMENTSTARTDATE = '2018-11-17 05:00:00.000'
						)
					THEN 'Global Response'
				WHEN left(PERSONNELNUMBER, 3) = 'GEO'
					THEN 'GEODIS'
				WHEN left(PERSONNELNUMBER, 2) = 'VN'
					THEN 'Visionet'
				WHEN left(PERSONNELNUMBER, 3) IN (
						'CWC'
						,'CWD'
						)
					THEN 'CWD Corporate'
				WHEN left(PERSONNELNUMBER, 3) = 'CWI'
					THEN 'CWI Corporate'
				WHEN left(PERSONNELNUMBER, 2) = 'VN'
					THEN 'Visionet'
				WHEN left(PERSONNELNUMBER, 2) = 'IQ'
					THEN 'iQor'
				END
			...
			,Seq = row_number() OVER (
				PARTITION BY e.PersonnelNumber ORDER BY ISNULL(u.Enabled, 0) DESC
					,SUMMARYVALIDFROM
				)
		FROM [AX_PRODUCTION].[dbo].[HcmEmployeeStaging] e
		LEFT JOIN [AX_PRODUCTION].[dbo].[SystemUserStaging] u ON ltrim(rtrim(u.username)) = ltrim(rtrim(e.name))
	...
	AS (
		SELECT [Personnel_Number]
			...
			,[Start_Date]
			,[End_Date] = CASE 
				WHEN DATEADD(DAY, - 1, LEAD([Start_Date], 1) OVER (
							PARTITION BY [Personnel_Number] ORDER BY [Start_Date] ASC
							)) < [Start_Date]
					THEN [Start_Date]
				ELSE DATEADD(DAY, - 1, LEAD([Start_Date], 1) OVER (
							PARTITION BY [Personnel_Number] ORDER BY [Start_Date] ASC
							))
				END
			,[Summary_Valid_From]
	...
	AS (
		SELECT *
		FROM [CWC_DW].[Dimension].[Employees]
		)
	MERGE INTO tgt
	USING src
		ON tgt.[Personnel_Number] = src.[Personnel_Number]
	...
	WHEN NOT MATCHED
		THEN
			...
			VALUES (
				src.[Personnel_Number]
				,src.[User_ID]
				,src.[Employee_Full_Name]
				,src.[License_Source]
				,src.[Start_Date]
				,src.[End_Date]
				,src.[Summary_Valid_From]
				,src.[is_Active]
	...
	WHEN MATCHED
		AND ISNULL(tgt.[User_ID], '') <> ISNULL(src.[User_ID], '')
