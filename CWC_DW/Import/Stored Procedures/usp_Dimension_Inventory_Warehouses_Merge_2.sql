...
CREATE PROC [Import].[usp_Dimension_Inventory_Warehouses_Merge]
AS
...
BEGIN TRY
	...
	WITH src
	AS (
		SELECT [Inventory_Warehouse_ID] = [WAREHOUSEID]
	...
	AS (
		SELECT *
		FROM [CWC_DW].[Dimension].[Inventory_Warehouses]
		)
	MERGE INTO tgt
	USING src
	...
	WHEN NOT MATCHED
		THEN
			...
			VALUES (
				...
				,src.[Inventory_Warehouse]
				,src.[State_ID]
				,src.[Zip_Code]
				,src.[is_Active]
				)
	WHEN MATCHED
		AND ISNULL(src.[Inventory_Warehouse], '') <> ISNULL(tgt.[Inventory_Warehouse], '')
		OR ISNULL(src.[State_ID], '') <> ISNULL(tgt.[State_ID], '')
		OR ISNULL(src.[Zip_Code], '') <> ISNULL(tgt.[Zip_Code], '')
		THEN
			UPDATE
			SET [Inventory_Warehouse] = src.[Inventory_Warehouse]
