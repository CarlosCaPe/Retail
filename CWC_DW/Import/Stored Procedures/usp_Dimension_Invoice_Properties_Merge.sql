...
(
	 SELECT DISTINCT
	  ...
	  FROM [AX_PRODUCTION].[dbo].[ITSSalesInvoiceHeaderStaging] ih (NOLOCK)
	  INNER JOIN [AX_PRODUCTION].[dbo].[ITSSalesInvoiceLineStaging] il (NOLOCK) ON ih.INVOICENUMBER = il.INVOICENUMBER
)

,tgt AS (SELECT * FROM [CWC_DW].[Dimension].[Invoice_Properties] (NOLOCK))
