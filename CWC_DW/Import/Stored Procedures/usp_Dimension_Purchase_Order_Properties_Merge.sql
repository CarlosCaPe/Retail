...
BEGIN TRY
	...
	WITH src
	AS (
		SELECT DISTINCT [Purchase_Order_Header_Status] = CASE p.[PURCHASEORDERSTATUS]
				WHEN 1
					THEN 'Open'
				WHEN 2
					THEN 'Received'
			...
			,[Purchase_Order_Header_Vendor_Payment_Method_Name] = CASE [VENDORPAYMENTMETHODNAME]
				...
				ELSE NULLIF([VENDORPAYMENTMETHODNAME], '')
				END
			,[Purchase_Order_Line_Status] = CASE pl.[PURCHASEORDERLINESTATUS]
				WHEN 1
					THEN 'Open'
				WHEN 2
					THEN 'Received'
	...
	AS (
		SELECT *
		FROM [CWC_DW].[Dimension].[Purchase_Order_Properties]
		)
	MERGE INTO tgt
	USING src
	...
	WHEN NOT MATCHED
		THEN
			...
			VALUES (
				src.[Purchase_Order_Header_Status]
				,src.[Purchase_Order_Header_Delivery_Mode_ID]
				,src.[Purchase_Order_Header_Delivery_Terms_ID]
				,src.[Purchase_Order_Header_Document_Approval_Status]
				,src.[Purchase_Order_Header_Buyer_Group_ID]
				,src.[Purchase_Order_Header_Payment_Terms_Name]
				,src.[Purchase_Order_Header_Vendor_Payment_Method_Name]
				,src.[Purchase_Order_Line_Status]
