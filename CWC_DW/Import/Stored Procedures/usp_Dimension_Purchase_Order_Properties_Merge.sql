...
CREATE PROC [Import].[usp_Dimension_Purchase_Order_Properties_Merge]
  ...
  SELECT DISTINCT
	...
	,[Purchase_Order_Header_Season] = s.Purchase_Order_Header_Season--CAST(NULLIF(REASONCOMMENT,'') AS VARCHAR(25))
...
LEFT JOIN [CWC_DW].[Reference].[Purchace_Order_Seasons] (NOLOCK) s ON p.REASONCODE = s.Purchase_Order_Header_Season_CD
) --SELECT * FROM src
,tgt AS (SELECT * FROM [CWC_DW].[Dimension].[Purchase_Order_Properties] (NOLOCK))
