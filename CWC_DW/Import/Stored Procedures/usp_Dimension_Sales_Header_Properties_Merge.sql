...
CREATE PROC [Import].[usp_Dimension_Sales_Header_Properties_Merge]
AS
...
BEGIN TRY
	...
	AS (
		SELECT DISTINCT
			...
			[Sales_Header_Type] = CASE so.[SALESTYPE]
				WHEN 2
					THEN 'Subscription'
				WHEN 3
					THEN 'Sale'
	...
	WHEN NOT MATCHED
		THEN
			INSERT (
				...
				,[Sales_Header_Retail_Channel_Name]
			...
			VALUES (
				src.[Sales_Header_Type]
				,src.[Sales_Header_Status]
				,src.[Sales_Header_Retail_Channel]
				,src.[Sales_Header_Retail_Channel_Name]
				,src.[Sales_Header_Return_Reason_Group]
				,src.[Sales_Header_Return_Reason]
				,src.[Sales_Header_Delivery_Mode]
				,src.[Sales_Header_On_Hold]
	...
	WHEN MATCHED
		...
		THEN
			UPDATE
			SET tgt.[is_Removed_From_Source] = 0
