...
--INTO [CWC_DW].[Dimension].[Sales_Lines_Attributes]
FROM [AX_PRODUCTION].[dbo].[ITSSalesOrderHeaderEntityStaging] s (NOLOCK)
INNER JOIN [AX_PRODUCTION].[dbo].[ITSSalesOrderLineStaging] sl (NOLOCK) ON s.[SALESORDERNUMBER] = sl.[SALESORDERNUMBER]
...
--AND CAST(s.Sales_Line_Created_Date_EST AS DATE) >= '12/1/2021'
--GROUP BY  slp.Sales_Line_Status
) --SELECT * FROM src
