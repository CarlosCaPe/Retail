...
CREATE PROC [Import].[usp_Dimension_Sales_Line_Properties_Merge]
AS
...
BEGIN TRY
	...
	WITH src
	AS (
		SELECT DISTINCT [Sales_Line_Status] = CASE sl.[SALESORDERLINESTATUS]
				WHEN 0
					THEN 'None'
				WHEN 1
					THEN 'Backorder'
		...
		SELECT DISTINCT Sales_Line_Status = CASE 
		...
		WHERE s.is_Removed_From_Source = 1
			AND slp2.Sales_Line_Property_Key IS NULL
		)
		,tgt
	AS (
		SELECT *
	...
	MERGE INTO tgt
	USING src
		ON ISNULL(tgt.[Sales_Line_Status], '') = ISNULL(src.[Sales_Line_Status], '')
			AND ISNULL(tgt.[Sales_Line_Type], '') = ISNULL(src.[Sales_Line_Type], '')
			AND ISNULL(tgt.[Sales_Line_Tax_Group], '') = ISNULL(src.[Sales_Line_Tax_Group], '')
			AND ISNULL(tgt.[Sales_Line_Unit], '') = ISNULL(src.[Sales_Line_Unit], '')
			AND ISNULL(tgt.[Sales_Line_Return_Deposition_Action], '') = ISNULL(src.[Sales_Line_Return_Deposition_Action], '')
			AND ISNULL(tgt.[Sales_Line_Return_Deposition_Description], '') = ISNULL(src.[Sales_Line_Return_Deposition_Description], '')
	WHEN NOT MATCHED
		THEN
			INSERT (
				[Sales_Line_Status]
			...
			VALUES (
				src.[Sales_Line_Status]
				,src.[Sales_Line_Type]
				,src.[Sales_Line_Tax_Group]
				,src.[Sales_Line_Unit]
				,src.[Sales_Line_Return_Deposition_Action]
				,src.[Sales_Line_Return_Deposition_Description]
	...
	WHEN MATCHED
		AND tgt.[is_Removed_From_Source] <> src.[is_Removed_From_Source]
		THEN
			UPDATE
			SET tgt.[is_Removed_From_Source] = src.[is_Removed_From_Source]
