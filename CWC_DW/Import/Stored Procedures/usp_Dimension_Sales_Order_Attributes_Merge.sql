...
BEGIN TRY
	...
	AS (
			...
			,[Order_Classification] = CASE 
				WHEN slp.Sales_Line_Type = 'Sale'
					AND (
						...
						AND s.is_Removed_From_Source <> 1
						)
					THEN 'Sale'
		...
		,src
	AS (
		SELECT DISTINCT [Order_Classification]
			,[Future_Return]
			,[On_Pick_List]
		FROM Order_Class
		) --SELECT * FROM src
	...
	AS (
		SELECT *
		FROM [Dimension].[Sales_Order_Attributes]
		)
	MERGE INTO tgt
	USING src
		ON tgt.[Order_Classification] = src.[Order_Classification]
			AND tgt.[Future_Return] = src.[Future_Return]
			AND tgt.[On_Pick_List] = src.[On_Pick_List]
	WHEN NOT MATCHED
		THEN
			INSERT (
