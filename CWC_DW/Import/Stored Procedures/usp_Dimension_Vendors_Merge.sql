...
SELECT --TOP (2000) 
	  ...
	  ,[Vendor_Group] = CASE VENDORGROUPID WHEN 'INV' THEN 'Invoice' WHEN 'NON-INV' THEN 'Non Invoice' ELSE VENDORGROUPID END
      ...
      ,[Default_Delivery_Mode] = CAST(CASE NULLIF([DEFAULTDELIVERYMODEID],'')
										...
										ELSE NULLIF([DEFAULTDELIVERYMODEID],'') END AS VARCHAR(50))
	  ...
	  ,[Default_Delivery_Terms] = CASE  NULLIF([DEFAULTDELIVERYTERMSCODE],'')
							...
							ELSE NULLIF([DEFAULTDELIVERYTERMSCODE],'') END
      ...
      ,[Default_Vendor_Payment_Method] = CASE NULLIF([DEFAULTVENDORPAYMENTMETHODNAME],'')
										...
										ELSE NULLIF([DEFAULTVENDORPAYMENTMETHODNAME],'') END
 ...
 -- TRUNCATE TABLE [CWC_DW].[Dimension].[Vendors]
  FROM [AX_PRODUCTION].[dbo].[VendVendorV2Staging] (NOLOCK) v
  LEFT JOIN [CWC_DW].[Reference].[Countries] (NOLOCK) c ON v.[ADDRESSCOUNTRYREGIONID] = c.Country_Region_ID
  --WHERE VENDORGROUPID = 'INV'
)
,tgt AS (SELECT * FROM [CWC_DW].[Dimension].[Vendors] (NOLOCK))

MERGE INTO tgt USING src
	ON tgt.[Vendor_Account_Number] = src.[Vendor_Account_Number]
		WHEN NOT MATCHED THEN
			INSERT([Vendor_Account_Number],[Vendor_Group],[Vendor_Organization_Name],[Default_Delivery_Mode_CD],[Default_Delivery_Mode],[Default_Delivery_Terms_CD],[Default_Delivery_Terms],[Default_Vendor_Payment_Method_CD],[Default_Vendor_Payment_Method],[Address_Description],[Address_State_ID]
		          ,[Address_Street],[Address_City],[Address_Zip_Code],[Address_Country_Region_ID],[Country_Name],[is_Removed_From_Source],[is_Excluded],[ETL_Created_Date],[ETL_Modified_Date],[ETL_Modified_Count])
			VALUES(src.[Vendor_Account_Number],src.[Vendor_Group],src.[Vendor_Organization_Name],src.[Default_Delivery_Mode_CD],src.[Default_Delivery_Mode],src.[Default_Delivery_Terms_CD],src.[Default_Delivery_Terms],src.[Default_Vendor_Payment_Method_CD],src.[Default_Vendor_Payment_Method],src.[Address_Description],src.[Address_State_ID]
		          ,src.[Address_Street],src.[Address_City],src.[Address_Zip_Code],src.[Address_Country_Region_ID],src.[Country_Name],src.[is_Removed_From_Source],src.[is_Excluded],src.[ETL_Created_Date],src.[ETL_Modified_Date],src.[ETL_Modified_Count])
		WHEN MATCHED AND
				     ISNULL(tgt.[Vendor_Group],'') <> ISNULL(src.[Vendor_Group],'')
