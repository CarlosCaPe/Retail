...
CREATE TABLE #Dates
	(
		...
		--,CONSTRAINT PK_Temp_Date PRIMARY KEY (Date_Key)
	);
...
SELECT 
 @Stop_Date = DATEADD(DAY,-1,d1.First_Date_of_Week)--,d1.Fiscal_Week_of_Year,d1.Fiscal_Year,d1.[Fiscal_Month_Year_Week_Name]
,@Start_Date = d2.First_Date_of_Week--,d2.Fiscal_Week_of_Year,d2.Fiscal_Year,d2.[Fiscal_Month_Year_Week_Name]
...
SELECT 
	...
	--,[Demand_12_Week] = SUM(s2.Sales_Line_Amount)
	--,[Demand_Current_Week] = SUM(s2.Sales_Line_Amount)
FROM [CWC_DW].[Power_BI].[v_Dim_Dates] d1

--LEFT JOIN [Power_BI].[v_Fact_Sales_Sales_With_Cancels] (NOLOCK) s ON CAST(s.[Sales_Line_Created_Date] AS DATE) BETWEEN d2.First_Date_of_Week AND DATEADD(DAY,-1,d1.First_Date_of_Week)
--LEFT JOIN [CWC_DW].[Power_BI].[v_Fact_Sales_Sales_With_Cancels] (NOLOCK) s2 ON CAST(s2.[Sales_Line_Created_Date] AS DATE) BETWEEN d1.First_Date_of_Week AND d1.Last_Date_of_Week
...
FROM #Dates (NOLOCK) d1
	LEFT JOIN [CWC_DW].[Power_BI].[v_Fact_Sales_Sales_With_Cancels] (NOLOCK) s ON CAST(s.[Sales_Line_Created_Date] AS DATE) = d1.Calendar_Date --BETWEEN d1.First_Date_of_Week AND d1.Last_Date_of_Week
...
FROM #Dates (NOLOCK) d1
	...
	LEFT JOIN [CWC_DW].[Power_BI].[v_Fact_Sales_Sales_With_Cancels] (NOLOCK) s ON CAST(s.[Sales_Line_Created_Date] AS DATE) BETWEEN d2.[First_Date_of_Week] AND DATEADD(DAY,-1,d1.[First_Date_of_Week])
WHERE is_Week_End = 1
GROUP BY d1.Date_Key
)
UPDATE d
...
(
  ...
  INNER JOIN [CWC_DW].[Power_BI].[v_Fact_Sales_Sales_With_Cancels] (NOLOCK) s ON s.Invent_Trans_ID = i.Invent_Trans_ID
  GROUP BY d.Date_Key
)
UPDATE d
SET
...
SELECT 
      ...
      --,[is_Backordered_Product] = SUM(CASE WHEN [is_Backordered_Product] = 0 THEN [Vendor_Packing_Slip_Quantity] END)
  FROM #Dates (NOLOCK) d
  INNER JOIN [CWC_DW].[Fact].[Vendor_Packing_Slips] (NOLOCK) vps ON vps.[Vendor_Packing_Accounting_Date] = d.Calendar_Date
  GROUP BY d.Date_Key
)
UPDATE d
SET
...
SELECT
	...
	--,[Backorder_Cancel_Amount] = SUM(Demand_Canceled)
FROM #Dates (NOLOCK) d  
	LEFT JOIN [CWC_DW].[Fact].[Backorders] (NOLOCK) b ON b.[Snapshot_Date_Key] = d.[Date_Key] AND (is_Week_Start = 1 OR is_Week_End = 1)
	--INNER JOIN [CWC_DW].[Power_BI].[v_Dim_Dates] (NOLOCK) bd ON b.[Snapshot_Date_Key] = bd.[Calendar_Date]
	--LEFT JOIN [CWC_DW].[Power_BI].[v_Dim_Dates] (NOLOCK) d12 ON DATEADD(DAY,-84,d.[Calendar_Date]) = d12.Calendar_Date
	--LEFT JOIN [CWC_DW].[Fact].[Sales] (NOLOCK) s ON CAST(s.[Sales_Line_Created_Date] AS DATE) BETWEEN d12.First_Date_of_Week AND DATEADD(DAY,-1,bd.Fir
	...
	--LEFT JOIN [CWC_DW].[Dimension].[Sales_Line_Properties] (NOLOCK) slp ON b.Sales_Line_Property_Key = slp.Sales_Line_Property_Key
	--LEFT JOIN [CWC_DW].[Dimension].[Sales_Header_Properties] (NOLOCK) shp ON b.Sales_Header_Property_Key = shp.Sales_Header_Property_Key
WHERE b.is_Removed_From_Source = 0
GROUP BY d.Date_Key,d.Fiscal_Week_Index,Fiscal_Week_Name
) --SELECT * FROM BO_Begin_End_Calc
,BO_Begin_End AS
(
SELECT
     d.[Fiscal_Week_Index]	
...
SELECT * 
--INTO [CWC_DW].[Fact].[Backorder_Fiscal_Week_Summary]
FROM BO_Begin_End 
--ORDER BY Fiscal_Week_Index DESC
;
