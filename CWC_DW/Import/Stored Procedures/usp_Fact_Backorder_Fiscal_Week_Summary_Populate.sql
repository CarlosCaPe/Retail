...
BEGIN TRY
	...
	SELECT @Stop_Date = DATEADD(DAY, - 1, d1.First_Date_of_Week)
	...
	SELECT d1.[Date_Key]
		...
		,[Fiscal_Week_Index] = DENSE_RANK() OVER (
			ORDER BY d1.[Fiscal_Year]
				,d1.[Fiscal_Week_of_Year]
			)
		,d1.First_Date_of_Week
		,d1.Last_Date_of_Week
		,[is_Week_Start] = CASE 
			WHEN d1.Calendar_Date <> d1.First_Date_of_Week
				THEN 0
			ELSE 1
			END
		,[is_Week_End] = CASE 
			WHEN d1.Calendar_Date <> d1.Last_Date_of_Week
				THEN 0
			ELSE 1
			END
	FROM [CWC_DW].[Power_BI].[v_Dim_Dates] d1
	WHERE d1.Calendar_Date BETWEEN @Start_Date
			AND @Stop_Date
	...
	AS (
		...
		LEFT JOIN [CWC_DW].[Power_BI].[v_Fact_Sales_Sales_With_Cancels] s ON CAST(s.[Sales_Line_Created_Date] AS DATE) BETWEEN d2.[First_Date_of_Week]
				AND DATEADD(DAY, - 1, d1.[First_Date_of_Week])
		WHERE is_Week_End = 1
		GROUP BY d1.Date_Key
		)
	UPDATE d
	...
	AS (
		...
		INNER JOIN [CWC_DW].[Fact].[Backorders] b ON d.Date_Key = b.Snapshot_Date_Key
			AND CAST(b.[Sales_Line_Created_Date] AS DATE) = d.Calendar_Date
		GROUP BY d.Date_Key
		)
	UPDATE d
	SET Backorder_Amount = b.Backorder_Amount
	...
	AS (
		SELECT d.Date_Key
			,Demand_Shipped_Amount = SUM(CASE 
					WHEN i.[Invoice_Line_Amount] > 0
						THEN CAST(i.[Invoice_Line_Amount] AS NUMERIC(18, 6))
					END)
			,Demand_Shipped_Quantity = SUM(CASE 
					WHEN i.[Invoice_Line_Amount] > 0
						THEN i.[Invoice_Line_Quantity]
					END)
		FROM #Dates d
	...
	UPDATE d
	SET d.Backorder_Shipped_Amount = s.Backorder_Shipped_Amount
	...
	AS (
		SELECT Date_Key
			...
			,Receipt_Backorder = SUM(CASE 
					WHEN [is_Backordered_Product] = 1
						THEN [Vendor_Packing_Slip_Quantity]
					END)
			,Receipt_Other = SUM(CASE 
					WHEN [is_Backordered_Product] = 0
						THEN [Vendor_Packing_Slip_Quantity]
					END)
		FROM #Dates d
	...
	UPDATE d
	SET d.Receipt_Other = r.Receipt_Other
	...
	UPDATE d
	SET d.Demand_Canceled = c.Cancel_Amount
	...
	AS (
		SELECT d.[Date_Key]
			...
			,[Beginning_Backorder_Amount] = SUM(CASE 
					WHEN [is_Week_Start] = 1
						THEN ([Sales_Line_Amount] / CAST([Sales_Line_Ordered_Sales_Quantity] AS NUMERIC(18, 6))) * CAST([Sales_Line_Remain_Sales_Physical] AS NUMERIC(18, 6))
					ELSE NULL
					END)
			,[Ending_Backorder_Amount] = SUM(CASE 
					WHEN [is_Week_End] = 1
						THEN ([Sales_Line_Amount] / CAST([Sales_Line_Ordered_Sales_Quantity] AS NUMERIC(18, 6))) * CAST([Sales_Line_Remain_Sales_Physical] AS NUMERIC(18, 6))
					ELSE NULL
					END)
			,[Beginning_Backorder_Quantity] = SUM(CASE 
					WHEN [is_Week_Start] = 1
						THEN [Sales_Line_Remain_Sales_Physical]
					ELSE NULL
					END)
			,[Ending_Backorder_Quantity] = SUM(CASE 
					WHEN [is_Week_End] = 1
						THEN [Sales_Line_Remain_Sales_Physical]
					ELSE NULL
					END)
			,[Demand_12_Week] = MAX(CASE 
					WHEN is_Week_End = 1
						THEN [Demand_12_Week]
					END)
		...
		FROM #Dates d
		LEFT JOIN [CWC_DW].[Fact].[Backorders] b ON b.[Snapshot_Date_Key] = d.[Date_Key]
			AND (
				is_Week_Start = 1
				OR is_Week_End = 1
				)
	...
	AS (
		SELECT d.[Fiscal_Week_Index]
			...
			,[New_Backorder_Rate] = CASE 
				WHEN SUM(d.[Demand]) > 0
					THEN SUM(d.Backorder_Amount) / SUM(d.[Demand])
				ELSE NULL
				END
			,[Backorder_Shipped_Amount] = SUM(d.Backorder_Shipped_Amount)
			,[Shipped_Backorder_Rate] = CASE 
				WHEN SUM(d.[Demand]) > 0
					THEN SUM(d.Backorder_Shipped_Amount) / SUM(d.[Demand])
				ELSE NULL
				END
			,[Backorder_Misc] = SUM([Beginning_Backorder_Amount]) + SUM(d.Backorder_Amount) - SUM(d.Backorder_Shipped_Amount) - SUM(d.Demand_Canceled) - SUM([Ending_Backorder_Amount])
			...
			,[Order_Fill_Rate] = CASE 
				WHEN SUM(d.[Orders]) <> 0
					THEN (SUM(d.[Orders]) - SUM([Orders_Backorder])) / CAST(SUM(d.[Orders]) AS NUMERIC(18, 6))
				ELSE NULL
				END
