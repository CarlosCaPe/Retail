...
BEGIN TRY
	...
	SELECT  @Current_Fiscal_Week_Year_Index =
		(SELECT MIN(Fiscal_Week_Year_Index) FROM #PO_Open  );

	SELECT  @Current_Fiscal_Week_Year =
		(SELECT TOP 1 Fiscal_Week_Year FROM #PO_Open   WHERE Fiscal_Week_Year_Index = @Current_Fiscal_Week_Year_Index);

	SELECT @Start_Fiscal_Week_Year_Index = @Current_Fiscal_Week_Year_Index;


	SELECT @Stop_Fiscal_Week_Year_Index = (SELECT MAX(Fiscal_Week_Year_Index) FROM #PO_Open  );
	...
	WHILE @Current_Fiscal_Week_Year_Index BETWEEN @Start_Fiscal_Week_Year_Index AND @Stop_Fiscal_Week_Year_Index
		BEGIN
			...
			FROM #Backorders_by_SKU   bo
			LEFT JOIN #PO_Open   po ON bo.Product_Key = po.Product_Key AND @Current_Fiscal_Week_Year_Index = po.Fiscal_Week_Year_Index
			;
			...
			INSERT INTO CWC_DW.Fact.Backorder_Relief_Summary
					...
					FROM #Backorders_by_SKU   bo
					LEFT JOIN #PO_Open   po ON bo.Product_Key = po.Product_Key AND bo.Fiscal_Week_Year_Index >= po.Fiscal_Week_Year_Index
					GROUP BY
						 bo.Product_Key
