...
BEGIN TRY
	...
	DELETE [CWC_DW].[Fact].[Backorders]
	WHERE Snapshot_Date_Key = @Snapshot_Date_Key;

	INSERT INTO CWC_DW.Fact.Backorders
	SELECT Snapshot_Date_Key = @Snapshot_Date_Key
		,s.*
	...
	SET [is_Current_Backorder_Date] = CASE 
			WHEN Date_Key = @Snapshot_Date_Key
				THEN 1
			ELSE 0
			END;
	...
	SET is_Excluded = CASE 
			WHEN p.Sales_Key IS NULL
				THEN 0
			ELSE 1
			END
	FROM [CWC_DW].[Fact].[Backorders] b
	INNER JOIN [CWC_DW].[Dimension].[Dates] d ON b.Snapshot_Date_Key = d.Date_Key
	LEFT JOIN Picks p ON p.Sales_Key = b.Sales_Key
		AND d.Calendar_Date > p.Created_Date
	WHERE b.is_Excluded <> CASE 
			WHEN p.Sales_Key IS NULL
				THEN 0
			ELSE 1
			END;
