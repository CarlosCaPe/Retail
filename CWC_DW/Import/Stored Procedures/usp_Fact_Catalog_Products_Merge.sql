...
--INTO [CWC_DW].[Fact].[Catalog_Products]
FROM [AX_PRODUCTION].[dbo].[RetailCatalogProductStaging] (NOLOCK) cp
INNER JOIN [CWC_DW].[Dimension].[Products] (NOLOCK) p ON p.[Product_Varient_Number] = cp.[DISPLAYPRODUCTNUMBER]
INNER JOIN [CWC_DW].[Dimension].[Catalogs] (NOLOCK) c ON c.Catalog_Number = cp.CATALOGNUMBER
)
,tgt AS (SELECT * FROM [CWC_DW].[Fact].[Catalog_Products] (NOLOCK))
