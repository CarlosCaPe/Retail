...
BEGIN TRY
	...
	WITH Declines
	AS (
		SELECT Sales_Order_Number = SalesID
			,Decline_Date = APPROVEDDATETIME
			,Authorization_Amount = cc.APPROVALAMOUNTCUR
			,Message_Description = cc.DESCRIPTION
			,Seq_First_Decline = ROW_NUMBER() OVER (
				PARTITION BY SalesID ORDER BY APPROVEDDATETIME
				)
			,Seq_Last_Decline = ROW_NUMBER() OVER (
				PARTITION BY SalesID ORDER BY APPROVEDDATETIME DESC
				)
		FROM [AX_PRODUCTION].[dbo].ITSCreditCardAuthTransStaging cc
		WHERE CAST(APPROVEDDATETIME AS DATE) > CAST('12/1/2020' AS DATE)
			AND DESCRIPTION LIKE '%Decline%'
		)
		,Sales_Orders
	AS (
		SELECT DISTINCT [Sales_Order_Number]
			...
			,Seq = ROW_NUMBER() OVER (
				PARTITION BY [Sales_Order_Number] ORDER BY Sales_Line_Created_Date
				)
	...
	AS (
		SELECT Sales_Order_Number = SalesID
			...
			,Seq_First_Success = ROW_NUMBER() OVER (
				PARTITION BY SalesID ORDER BY APPROVEDDATETIME
				)
			,Seq_Last_Success = ROW_NUMBER() OVER (
				PARTITION BY SalesID ORDER BY APPROVEDDATETIME DESC
				)
		FROM [AX_PRODUCTION].[dbo].ITSCreditCardAuthTransStaging cc
		WHERE CAST(APPROVEDDATETIME AS DATE) > cast('12/1/2020' AS DATE)
			AND DESCRIPTION LIKE '%Successful%'
		)
		,First_Success
	AS (
		SELECT Sales_Order_Number
	...
	AS (
		...
		,src
	AS (
		SELECT Sales_Order_Number = CAST(fd.Sales_Order_Number AS VARCHAR(25))
			,Credit_Card_Authorization_Key = CASE 
				WHEN ls.Last_Success_Date > ld.Last_Decline_Date
					THEN 1
				WHEN ls.Last_Success_Date < ld.Last_Decline_Date
					THEN 3
				ELSE 2
	...
	AS (
		SELECT *
		FROM [CWC_DW].[Fact].[Credit_Card_Declines]
		)
	MERGE INTO tgt
	USING src
	...
	WHEN NOT MATCHED
		THEN
			INSERT (
				...
				,[First_Decline_Date]
				,[Last_Decline_Date]
				,[First_Decline_Amount]
				,[Last_Decline_Amount]
				,[Success_Count]
				,[First_Success_Date]
				,[Last_Success_Date]
				,[First_Success_Amount]
				,[Last_Success_Amount]
			...
			VALUES (
				...
				,src.[Credit_Card_Authorization_Key]
				,src.[Sales_Header_Customer_Key]
				,src.[Sales_Header_Property_Key]
				,src.[Sales_Header_Location_Key]
				,src.[Sales_Header_Created_Date]
				,src.[Decline_Count]
				,src.[First_Decline_Date]
				,src.[Last_Decline_Date]
				,src.[First_Decline_Amount]
				,src.[Last_Decline_Amount]
				,src.[Success_Count]
				,src.[First_Success_Date]
				,src.[Last_Success_Date]
				,src.[First_Success_Amount]
				,src.[Last_Success_Amount]
	...
	WHEN MATCHED
		AND ISNULL(tgt.[Credit_Card_Authorization_Key], '') <> ISNULL(src.[Credit_Card_Authorization_Key], '')
		...
		THEN
			UPDATE
			SET [Credit_Card_Authorization_Key] = src.[Credit_Card_Authorization_Key]
