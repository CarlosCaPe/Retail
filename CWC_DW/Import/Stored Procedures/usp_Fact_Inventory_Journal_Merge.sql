...
BEGIN TRY
	...
	WITH src AS
		(
			...
			INNER JOIN [CWC_DW].[Dimension].[Inventory_Journal_Properties]   ijp ON ij.COUNTINGREASONCODE =ijp.[Counting_Reason_CD] 																				AND ISNULL(ij.INVENTSTATUSID,'') = ISNULL(ijp.[Inventory_Status_CD],'')
	...
	,tgt AS (SELECT * FROM [CWC_DW].[Fact].[Inventory_Journals]  )
