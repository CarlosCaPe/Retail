...
BEGIN TRY
	...
	AS (
		...
		INNER JOIN CWC_DW.Dimension.Products p ON its.PRODUCTCOLORID = p.Color_ID
			AND its.PRODUCTSIZEID = p.Size_ID
			AND its.ITEMNUMBER = p.Item_ID
		...
		,AV
	AS (
		SELECT [Product_Key] = p.Product_Key
		...
		FROM [AX_PRODUCTION].[dbo].[ITSInventWarehouseInventoryStatusOnHandStaging] its
		INNER JOIN Product_List p ON its.PRODUCTCOLORID = p.Color_ID
			AND its.PRODUCTSIZEID = p.Size_ID
			AND its.ITEMNUMBER = p.Item_ID
			AND p.Inventory_Warehouse_ID = its.INVENTORYWAREHOUSEID
		LEFT JOIN [WMS_INV].[dbo].[FACT_WMS_INVENTORY] wms ON wms.[PRODUCTVARIANTNUMBER] = p.SKU
			AND wms.WAREHOUSE_ID = p.Inventory_Warehouse_ID
			AND wms.INVENTORY_STATUS_ID = its.INVENTORYSTATUSID
	...
	AS (
		...
		,src
	AS (
		SELECT [Snapshot_Date_Key] = @Snapshot_Date
		...
		--INTO Fact.Inventory
		FROM Product_List p
		LEFT JOIN AV ON p.Product_Key = av.Product_Key
			AND p.Inventory_Warehouse_Key = av.Inventory_Warehouse_Key
		LEFT JOIN RT ON p.Product_Key = rt.Product_Key
			AND p.Inventory_Warehouse_Key = rt.Inventory_Warehouse_Key
		LEFT JOIN DM ON p.Product_Key = dm.Product_Key
			AND p.Inventory_Warehouse_Key = dm.Inventory_Warehouse_Key
		LEFT JOIN DA ON p.Product_Key = da.Product_Key
			AND p.Inventory_Warehouse_Key = da.Inventory_Warehouse_Key
		LEFT JOIN RH ON p.Product_Key = rh.Product_Key
			AND p.Inventory_Warehouse_Key = rh.Inventory_Warehouse_Key
		LEFT JOIN SU ON p.Product_Key = su.Product_Key
			AND p.Inventory_Warehouse_Key = su.Inventory_Warehouse_Key
		LEFT JOIN WD ON p.Product_Key = wd.Product_Key
			AND p.Inventory_Warehouse_Key = wd.Inventory_Warehouse_Key
		LEFT JOIN BL ON p.Product_Key = bl.Product_Key
			AND p.Inventory_Warehouse_Key = bl.Inventory_Warehouse_Key
		LEFT JOIN GQ ON p.Product_Key = gq.Product_Key
			AND p.Inventory_Warehouse_Key = gq.Inventory_Warehouse_Key
		) --SELECT * FROM src
		,tgt
	AS (
		SELECT *
		FROM [CWC_DW].[Fact].[Inventory]
		WHERE Snapshot_Date_Key = @Snapshot_Date
		)
	MERGE INTO tgt
	USING src
	...
	WHEN NOT MATCHED
		THEN
			INSERT (
				...
				,[ETL_Modified_Date]
				,[ETL_Modified_Count]
				,[Available_WMS_On_Hand_Quantity]
				,[Returns_WMS_On_Hand_Quantity]
				,[Damaged_WMS_On_Hand_Quantity]
				,[Damaged_But_Available_WMS_On_Hand_Quantity]
				,[Receipt_Hold_Available_WMS_On_Hand_Quantity]
				,[Temporary_WMS_On_Hand_Quantity]
				,[Warehouse_Damage_WMS_On_Hand_Quantity]
				,[Blocking_WMS_On_Hand_Quantity]
				,[General_Quarantine_WMS_On_Hand_Quantity]
				)
			VALUES (
				src.[Snapshot_Date_Key]
				,src.[Product_Key]
				,src.[Inventory_Warehouse_Key]
				,src.[Available_On_Hand_Quantity]
				,src.[Available_Reserved_On_Hand_Quantity]
				,src.[Available_Available_On_Hand_Quantity]
				,src.[Available_Ordered_Quantiity]
				,src.[Available_Reserved_Ordered_Quantity]
				,src.[Available_Available_Ordered_Quantity]
				,src.[Available_On_Order_Quantity]
				,src.[Available_Total_Available_Quantity]
				,src.[Available_Average_Cost]
				,src.[Returns_On_Hand_Quantity]
				,src.[Returns_Reserved_On_Hand_Quantity]
				,src.[Returns_Available_On_Hand_Quantity]
				,src.[Returns_Ordered_Quantiity]
				,src.[Returns_Reserved_Ordered_Quantity]
				,src.[Returns_Available_Ordered_Quantity]
				,src.[Returns_On_Order_Quantity]
				,src.[Returns_Total_Available_Quantity]
				,src.[Returns_Average_Cost]
				,src.[Damaged_On_Hand_Quantity]
				,src.[Damaged_Reserved_On_Hand_Quantity]
				,src.[Damaged_Available_On_Hand_Quantity]
				,src.[Damaged_Ordered_Quantiity]
				,src.[Damaged_Reserved_Ordered_Quantity]
				,src.[Damaged_Available_Ordered_Quantity]
				,src.[Damaged_On_Order_Quantity]
				,src.[Damaged_Total_Available_Quantity]
				,src.[Damaged_Average_Cost]
				,src.[Damaged_But_Available_On_Hand_Quantity]
				,src.[Damaged_But_Available_Reserved_On_Hand_Quantity]
				,src.[Damaged_But_Available_Available_On_Hand_Quantity]
				,src.[Damaged_But_Available_Ordered_Quantiity]
				,src.[Damaged_But_Available_Reserved_Ordered_Quantity]
				,src.[Damaged_But_Available_Available_Ordered_Quantity]
				,src.[Damaged_But_Available_On_Order_Quantity]
				,src.[Damaged_But_Available_Total_Available_Quantity]
				,src.[Damaged_But_Available_Average_Cost]
				,src.[Receipt_Hold_On_Hand_Quantity]
				,src.[Receipt_Hold_Reserved_On_Hand_Quantity]
				,src.[Receipt_Hold_Available_On_Hand_Quantity]
				,src.[Receipt_Hold_Ordered_Quantiity]
				,src.[Receipt_Hold_Reserved_Ordered_Quantity]
				,src.[Receipt_Hold_Available_Ordered_Quantity]
				,src.[Receipt_Hold_On_Order_Quantity]
				,src.[Receipt_Hold_Total_Available_Quantity]
				,src.[Receipt_Hold_Average_Cost]
				,src.[Temporary_On_Hand_Quantity]
				,src.[Temporary_Reserved_On_Hand_Quantity]
				,src.[Temporary_Available_On_Hand_Quantity]
				,src.[Temporary_Ordered_Quantiity]
				,src.[Temporary_Reserved_Ordered_Quantity]
				,src.[Temporary_Available_Ordered_Quantity]
				,src.[Temporary_On_Order_Quantity]
				,src.[Temporary_Total_Available_Quantity]
				,src.[Temporary_Average_Cost]
				,src.[Warehouse_Damage_On_Hand_Quantity]
				,src.[Warehouse_Damage_Reserved_On_Hand_Quantity]
				,src.[Warehouse_Damage_Available_On_Hand_Quantity]
				,src.[Warehouse_Damage_Ordered_Quantiity]
				,src.[Warehouse_Damage_Reserved_Ordered_Quantity]
				,src.[Warehouse_Damage_Available_Ordered_Quantity]
				,src.[Warehouse_Damage_On_Order_Quantity]
				,src.[Warehouse_Damage_Total_Available_Quantity]
				,src.[Warehouse_Damage_Average_Cost]
				,src.[Blocking_On_Hand_Quantity]
				,src.[Blocking_Reserved_On_Hand_Quantity]
				,src.[Blocking_Available_On_Hand_Quantity]
				,src.[Blocking_Ordered_Quantiity]
				,src.[Blocking_Reserved_Ordered_Quantity]
				,src.[Blocking_Available_Ordered_Quantity]
				,src.[Blocking_On_Order_Quantity]
				,src.[Blocking_Total_Available_Quantity]
				,src.[Blocking_Average_Cost]
				,src.[General_Quarantine_On_Hand_Quantity]
				,src.[General_Quarantine_Reserved_On_Hand_Quantity]
				,src.[General_Quarantine_Available_On_Hand_Quantity]
				,src.[General_Quarantine_Ordered_Quantiity]
				,src.[General_Quarantine_Reserved_Ordered_Quantity]
				,src.[General_Quarantine_Available_Ordered_Quantity]
				,src.[General_Quarantine_On_Order_Quantity]
				,src.[General_Quarantine_Total_Available_Quantity]
				,src.[General_Quarantine_Average_Cost]
				,src.[is_Removed_From_Source]
				,src.[is_Excluded]
				,src.[ETL_Created_Date]
				,src.[ETL_Modified_Date]
				,src.[ETL_Modified_Count]
				,src.[Available_WMS_On_Hand_Quantity]
				,src.[Returns_WMS_On_Hand_Quantity]
				,src.[Damaged_WMS_On_Hand_Quantity]
				,src.[Damaged_But_Available_WMS_On_Hand_Quantity]
				,src.[Receipt_Hold_Available_WMS_On_Hand_Quantity]
				,src.[Temporary_WMS_On_Hand_Quantity]
				,src.[Warehouse_Damage_WMS_On_Hand_Quantity]
				,src.[Blocking_WMS_On_Hand_Quantity]
				,src.[General_Quarantine_WMS_On_Hand_Quantity]
				)
	WHEN MATCHED
		AND ISNULL(tgt.[Available_On_Hand_Quantity], '') <> src.[Available_On_Hand_Quantity]
		...
		THEN
			UPDATE
			SET [Available_On_Hand_Quantity] = src.[Available_On_Hand_Quantity]
	...
	SET [is_Current_Inventory_Date] = CASE 
			WHEN Date_Key = @Snapshot_Date
				THEN 1
			ELSE 0
			END;
