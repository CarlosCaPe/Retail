...
(
 SELECT
      ...
      --,its.[INVENTORYSITEID]
      --,its.[INVENTORYWAREHOUSEID]
      --,its.[INVENTORYSTATUSID]
...
FROM [AX_PRODUCTION].[dbo].[ITSInventWarehouseInventoryStatusOnHandStaging] its
	...
	LEFT JOIN [WMS_INV].[dbo].[FACT_WMS_INVENTORY] (NOLOCK) wms ON wms.[PRODUCTVARIANTNUMBER] = p.SKU AND wms.WAREHOUSE_ID = p.Inventory_Warehouse_ID AND wms.INVENTORY_STATUS_ID =its.INVENTORYSTATUSID
...
(
 SELECT
      ...
      --,its.[AREWAREHOUSEMANAGEMENTPROCESSESUSED]
...
WHEN MATCHED AND
		...
		--OR tgt.[is_Excluded] <> src.[is_Excluded]
		--OR tgt.[ETL_Created_Date] <> src.[ETL_Created_Date]
		--OR tgt.[ETL_Modified_Date] <> src.[ETL_Modified_Date]
		--OR tgt.[ETL_Modified_Count] <> src.[ETL_Modified_Count]
THEN UPDATE SET
