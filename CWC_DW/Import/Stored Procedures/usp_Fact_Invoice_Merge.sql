...
CREATE PROC [Import].[usp_Fact_Invoice_Merge]
AS
...
BEGIN TRY
	...
	WITH Inv_Mapping
	AS (
		SELECT [Invoice_Number] = ih.[INVOICENUMBER]
		...
		LEFT JOIN [CWC_DW].[Dimension].[Products] p ON il.[PRODUCTNUMBER] = CONCAT (
				p.Item_ID
				,p.Color_ID
				,p.Size_ID
				)
	...
	USING src
		ON tgt.[Invoice_Number] = src.[Invoice_Number]
			AND tgt.[Invoice_Line_Number] = src.[Invoice_Line_Number]
	WHEN NOT MATCHED
		THEN
			INSERT (
				[Invoice_Number]
			...
			VALUES (
				...
				,src.[Invent_Trans_ID]
				,src.[Invoice_Line_Product_Key]
				,src.[Invoice_Line_Product_Number]
				,src.[Invoice_Customer_Key]
				,src.[Invoice_Customer_Account_Number]
				,src.[Invoice_Property_Key]
				,src.[Invoice_Header_Date]
				,src.[Invoice_Line_Date_Key]
				,src.[Invoice_Line_Date]
				,src.[Invoice_Line_Confirmed_Shipping_Date]
				,src.[Invoice_Line_Delivery_Date]
				,src.[Invoice_Line_Quantity]
				,src.[Invoice_Line_Sales_Price]
				,src.[Invoice_Line_Amount]
				,src.[Invoice_Line_Tax_Amount]
				,src.[Invoice_Line_Charge_Amount]
				,src.[Invoice_Line_Discount_Amount]
	...
	WHEN MATCHED
		AND ISNULL(src.[Sales_Order_Number], '') <> ISNULL(tgt.[Sales_Order_Number], '')
		...
		THEN
			UPDATE
			SET [Sales_Order_Number] = src.[Sales_Order_Number]
