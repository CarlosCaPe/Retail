...
(
	...
	FROM [Inv_Mapping] i
	LEFT JOIN [CWC_DW].[Dimension].[Invoice_Properties] ip (NOLOCK) ON ISNULL(i.Payment_Term,'') = ISNULL(ip.Payment_Term,'') 
	...
	LEFT JOIN [CWC_DW].[Fact].[Sales] s (NOLOCK) ON i.[Invent_Trans_ID] = s.[Invent_Trans_ID]
) --SELECT * FROM src
