...
CREATE PROC [Import].[usp_Fact_Purchase_Orders_Merge]
AS
...
BEGIN TRY
	...
	AS (
		SELECT --TOP (1000) 
			...
			,[Purchase_Order_Header_Confirmed_Delivery_Date] = CAST(NULLIF([CONFIRMEDDELIVERYDATE], '1900-01-01 00:00:00.000') AS DATE)
			,[Purchase_Order_Header_Requested_Delivery_Date] = CAST(NULLIF([REQUESTEDDELIVERYDATE], '1900-01-01 00:00:00.000') AS DATE)
			,[Purchase_Order_Status] = CASE p.[PURCHASEORDERSTATUS]
				WHEN 1
					THEN 'Open'
				WHEN 2
					THEN 'Received'
			...
			,[Delivery_Mode_ID] = CAST(CASE NULLIF([DELIVERYMODEID], '')
					...
					WHEN 'ACNS'
						THEN 'Air China Shanghai'
					ELSE NULLIF([DELIVERYMODEID], '')
					END AS VARCHAR(50))
		...
		LEFT JOIN [CWC_DW].[Dimension].[Locations] l ON ISNULL(l.[Street], '') = ISNULL(CAST(NULLIF(p.[DELIVERYADDRESSSTREET], '') AS VARCHAR(200)), '')
		...
		LEFT JOIN [CWC_DW].[Dimension].[Vendors] v ON v.[Vendor_Account_Number] = p.[ORDERVENDORACCOUNTNUMBER]
		)
		,src
	AS (
		...
		INNER JOIN [AX_PRODUCTION].[dbo].[PurchPurchaseOrderLineV2Staging] pl ON p.[Purchase_Order_Number] = pl.PURCHASEORDERNUMBER
		LEFT JOIN [CWC_DW].[Dimension].[Products] prod ON ISNULL(pl.[ITEMNUMBER], '') = ISNULL(prod.Item_ID, '')
			AND ISNULL(pl.PRODUCTCOLORID, '') = ISNULL(prod.Color_ID, '')
			AND ISNULL(pl.PRODUCTSIZEID, '') = ISNULL(prod.Size_ID, '')
		LEFT JOIN [CWC_DW].[Dimension].[Purchase_Order_Properties] pop ON ISNULL(p.[Purchase_Order_Status], '') = ISNULL(pop.[Purchase_Order_Header_Status], '')
			...
			AND CASE pl.[PURCHASEORDERLINESTATUS]
				WHEN 1
					THEN 'Open'
				WHEN 2
					THEN 'Received'
	...
	WHEN NOT MATCHED
		THEN
			...
			VALUES (
				...
				,src.[Vendor_Key]
				,src.[Purchase_Order_Property_Key]
				,src.[Product_Key]
				,src.[Purchase_Order_Name]
				,src.[Purchase_Line_Description]
				,src.[Purchase_Order_Header_Accounting_Date]
				,src.[Purchase_Line_Confirmed_Delivery_Date]
				,src.[Purchase_Line_Requested_Delivery_Date]
				,src.[Purchase_Line_Confirmed_Shipping_Date]
				,src.[Purchase_Line_Requested_Shipping_Date]
				,src.[Purchase_Line_Amount]
				,src.[Purchase_Line_Purchase_Price]
				,src.[Purchase_Line_Discount_Amount]
				,src.[Purchase_Line_Price_Quantity]
				,src.[Purchase_Line_Ordered_Quantity]
				,src.[Received_Quantity]
				,src.[Vendor_Packing_Slip_Count]
				,src.[Barcode]
