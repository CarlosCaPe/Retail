...
CREATE PROC [Import].[usp_Fact_Purchase_Orders_Purchase_Order_Fill_Key_Update]
AS
...
BEGIN TRY
	...
	AS (
		SELECT po.Purchase_Order_Number
			,No_Fill_Count = SUM(CASE 
					WHEN [Purchase_Order_Line_Fill_CD] = 1
						AND pop.Purchase_Order_Line_Status <> 'Canceled'
						THEN 1
					ELSE 0
					END)
			,Partial_Fill_Count = SUM(CASE 
					WHEN [Purchase_Order_Line_Fill_CD] = 2
						AND pop.Purchase_Order_Line_Status <> 'Canceled'
						THEN 1
					ELSE 0
					END)
			,Complete_Fill_Count = SUM((
					CASE 
						WHEN [Purchase_Order_Line_Fill_CD] = 3
							AND pop.Purchase_Order_Line_Status <> 'Canceled'
							THEN 1
						ELSE 0
						END
			...
			,Over_Fill_Count = SUM((
					CASE 
						WHEN [Purchase_Order_Line_Fill_CD] = 4
							AND pop.Purchase_Order_Line_Status <> 'Canceled'
							THEN 1
						ELSE 0
						END
