...
WITH Commission AS
	(
		...
		FROM [CWC_DW].[Fact].[Purchase_Orders] (NOLOCK) po
			INNER JOIN [AX_PRODUCTION].[dbo].[PurchPurchaseOrderLineChargeStaging] (NOLOCK) pc ON pc.PURCHASEORDERNUMBER = po.Purchase_Order_Number AND pc.PURCHASEORDERLINENUMBER = po.Purchase_Order_Line_Number
