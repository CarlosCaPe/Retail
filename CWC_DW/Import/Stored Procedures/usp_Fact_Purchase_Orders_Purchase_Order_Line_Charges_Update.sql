...
BEGIN TRY
	...
	AS (
		SELECT [Purchase_Order_Key]
			...
			,[Seq] = ROW_NUMBER() OVER (
				PARTITION BY [Purchase_Order_Key] ORDER BY pc.CHARGELINENUMBER DESC
				)
		FROM [CWC_DW].[Fact].[Purchase_Orders] po
		INNER JOIN [AX_PRODUCTION].[dbo].[PurchPurchaseOrderLineChargeStaging] pc ON pc.PURCHASEORDERNUMBER = po.Purchase_Order_Number
			AND pc.PURCHASEORDERLINENUMBER = po.Purchase_Order_Line_Number
	...
	AS (
		...
		FROM [CWC_DW].[Fact].[Purchase_Orders] po
		LEFT JOIN Commission c ON po.[Purchase_Order_Key] = c.[Purchase_Order_Key]
			AND c.[Seq] = 1
		LEFT JOIN Tariff t ON po.[Purchase_Order_Key] = t.[Purchase_Order_Key]
			AND t.[Seq] = 1
		LEFT JOIN Duty d ON po.[Purchase_Order_Key] = d.[Purchase_Order_Key]
			AND d.[Seq] = 1
		LEFT JOIN Freight f ON po.[Purchase_Order_Key] = f.[Purchase_Order_Key]
			AND f.[Seq] = 1
		LEFT JOIN Freight_In fi ON po.[Purchase_Order_Key] = fi.[Purchase_Order_Key]
			AND fi.[Seq] = 1
		)
	UPDATE p
	SET [Commission_Charge_Percentage] = plc.[Commission_Charge_Percentage]
	...
	WHERE (
			(p.[Commission_Charge_Percentage] IS NULL)
			OR (p.[Commission_Charge_Percentage] <> plc.[Commission_Charge_Percentage])
			)
		OR (
			(p.[Tariff_Charge_Percentage] IS NULL)
			OR (p.[Tariff_Charge_Percentage] <> plc.[Tariff_Charge_Percentage])
			)
		OR (
			(p.[Duty_Charge_Percentage] IS NULL)
			OR (p.[Duty_Charge_Percentage] <> plc.[Duty_Charge_Percentage])
			)
		OR (
			(p.[Freight_Charge_Percentage] IS NULL)
			OR (p.[Freight_Charge_Percentage] <> plc.[Freight_Charge_Percentage])
			)
		OR (
			(p.[Freight_In_Charge_Percentage] IS NULL)
			OR (p.[Freight_In_Charge_Percentage] <> plc.[Freight_In_Charge_Percentage])
			);
