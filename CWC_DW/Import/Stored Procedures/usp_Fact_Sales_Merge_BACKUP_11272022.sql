...
CREATE PROC [Import].[usp_Fact_Sales_Merge_BACKUP_11272022]
AS
...
BEGIN TRY
	...
	WITH Sales_Order_Header
	AS (
		SELECT DISTINCT [Sales_Order_Number] = [SALESORDERNUMBER]
			,[Sales_Header_Customer_Key] = c.Customer_Key
			,[Sales_Header_Location_Key] = l.Location_Key
			,[Sales_Header_Currency_Code] = [CURRENCYCODE]
			,[Sales_Header_Retail_Channel] = CASE 
				WHEN LEFT([RETAILCHANNELID], 1) = 0
					THEN 'Retail'
				WHEN [RetailChannelID] IN (
						'9000'
						,'9001'
						)
					THEN 'Direct'
				ELSE rc.NAME
				END
			,[Sales_Header_Retail_Channel_Name] = CASE [Name]
				WHEN 'Ecommerce'
					THEN 'Web'
				WHEN 'Direct'
					THEN 'Phone'
				ELSE [NAME]
				END
			,[Sales_Header_Type] = CAST(CASE so.[SALESTYPE]
					WHEN 2
						THEN 'Subscription'
					WHEN 3
						THEN 'Sale'
			...
			,[Sales_Header_Return_Reason] = CAST(CASE so.[RETURNREASONCODEID]
					...
					WHEN '02'
						THEN 'Too Big'
					WHEN '03'
						THEN 'Item is Defective'
					WHEN '04'
						THEN 'Fabric/Material'
					WHEN '05'
						THEN 'Wrong Item Shipped'
					WHEN '06'
						THEN 'Arrived Too Late'
					WHEN '07'
						THEN 'Item Not as Described'
					WHEN '08'
						THEN 'Returning a Gift'
					WHEN '09'
						THEN 'Ordered Multiples'
					WHEN '10'
						THEN 'Changed Mind'
					WHEN '11'
						THEN 'Lost/Undelivered'
					WHEN '12'
						THEN 'Missing Item'
					WHEN '13'
					...
					WHEN '29'
						THEN 'Too Short'
					WHEN '30'
						THEN 'Ordered Wrong Size'
					WHEN '31'
						THEN 'Construction'
					WHEN '32'
						THEN 'Do Not Like Color'
					WHEN '33'
						THEN 'Damaged In Transit'
					WHEN '34'
						THEN 'Did Not Hold Up'
					WHEN '35'
						THEN 'Just Did Not Like'
					WHEN 'BIG'
						THEN 'Too Big'
					WHEN 'CM'
						THEN 'Changed Mind'
			...
			,[Sales_Header_Created_Date] = CASE 
				WHEN SALESCREATEDDATETIME = '1900-01-01 00:00:00.000'
					THEN SALESTABLECREATEDDATETIME
				ELSE SALESCREATEDDATETIME
				END --CAST(so.[SALESTABLECREATEDDATETIME] AS DATETIME2(5))
		...
		LEFT JOIN [CWC_DW].[Dimension].[Locations] l ON ISNULL(l.[Street], '') = ISNULL(CAST(NULLIF(so.[DELIVERYADDRESSSTREET], '') AS VARCHAR(200)), '')
	...
	AS (
		SELECT s.[Sales_Order_Number]
			...
			,[Sales_Line_Status] = CASE sl.[SALESORDERLINESTATUS]
				...
				WHEN 4
					THEN 'Canceled'
				END
			,[Sales_Line_Type] = CASE sl.SALESTYPE
				WHEN 2
					THEN 'Subscription'
				WHEN 3
					THEN 'Sale'
			...
			,[Sales_Line_Tax_Group] = CASE sl.[SALESTAXGROUPCODE]
				...
				WHEN ''
					THEN NULL
				WHEN 'EX'
					THEN 'Exempt from Sales Tax'
				WHEN 'IA'
					THEN 'Iowa State Sales Tax'
				WHEN 'IL'
					THEN 'Illinoise State Sales Tax'
				WHEN 'IN'
					THEN 'Indiana State Sales Tax'
				WHEN 'LKS'
					THEN 'Leawood, KS'
				WHEN 'MA'
					THEN 'Massachusetts Sales Tax'
				WHEN 'MI'
					THEN 'Michigan State Sales Tax'
				WHEN 'NM'
					THEN 'New Mexico Sales Tax'
				WHEN 'OH'
					THEN 'Ohio State Sales Tax'
				WHEN 'OK Tulsa'
					THEN 'Tulsa OK Tax Group'
				WHEN 'OKC'
					THEN 'Oklahoma City, OK'
				WHEN 'TX'
					THEN 'Texas State Sales Tax'
				END
			,[Sales_Line_Return_Deposition_Action] = CASE sl.[RETURNDISPOSITIONCODEID]
				...
				WHEN 'CRO'
					THEN 'Credit only'
				WHEN 'RAS'
					THEN 'Replace and scrap'
				WHEN 'RCR'
					THEN 'Replace and credit'
				WHEN 'RTC'
					THEN 'Return to customer'
				WHEN 'SCR'
					THEN 'Scrap'
				END
			,[Sales_Line_Return_Deposition_Description] = CASE sl.[RETURNDISPOSITIONCODEID]
				WHEN '01'
					THEN 'Store Return'
				WHEN '02'
					THEN 'Missing item from order'
				WHEN '03'
					THEN 'Order not received'
				WHEN '04'
					THEN 'Order discount adjustment'
				WHEN '05'
					THEN 'Wrong item received'
				WHEN '06'
					THEN 'Not my order'
				WHEN '07'
					THEN 'Refunded outbound shipping fee'
				WHEN '08'
					THEN 'Refunded return (6.95) shipping fee'
				WHEN '09'
					THEN 'Price adjustment'
				WHEN '10'
					THEN 'Defective item'
				WHEN '11'
					THEN 'Not posted return'
				WHEN '12'
					THEN 'General appeasement'
				WHEN 'CR'
					THEN 'Credit'
				WHEN 'CRO'
					THEN 'Credit only'
			...
			,[Sales_Line_Unit] = CASE sl.[SALESUNITSYMBOL]
				WHEN 'ea'
					THEN 'Each'
				WHEN ''
					THEN NULL
		...
		LEFT JOIN [CWC_DW].[Dimension].[Sales_Header_Properties] sha ON CAST(ISNULL(s.[Sales_Header_Type], '') AS VARCHAR(20)) = CAST(ISNULL(sha.[Sales_Header_Type], '') AS VARCHAR(20))
	...
	AS (
		SELECT [Sales_Order_Number]
			...
			--,[Sales_Line_Status] = ISNULL(s.[Sales_Line_Status],'')
			--   ,[Sales_Line_Type] = ISNULL(s.[Sales_Line_Type],'')
			--   ,[Sales_Line_Tax_Group] = ISNULL(s.[Sales_Line_Tax_Group],'')
			--,[Sales_Line_Unit] = ISNULL(s.[Sales_Line_Unit],'')
			--   ,[Sales_Line_Return_Deposition_Action] = ISNULL(s.[Sales_Line_Return_Deposition_Action],'')
			--   ,[Sales_Line_Return_Deposition_Description] = ISNULL(s.[Sales_Line_Return_Deposition_Description],'')
		...
		LEFT JOIN [CWC_DW].[Dimension].[Sales_Line_Properties] sla ON ISNULL(s.[Sales_Line_Status], '') = ISNULL(sla.[Sales_Line_Status], '')
			AND ISNULL(s.[Sales_Line_Type], '') = ISNULL(sla.[Sales_Line_Type], '')
			AND ISNULL(s.[Sales_Line_Tax_Group], '') = ISNULL(sla.[Sales_Line_Tax_Group], '')
			AND ISNULL(s.[Sales_Line_Unit], '') = ISNULL(sla.[Sales_Line_Unit], '')
			AND ISNULL(s.[Sales_Line_Return_Deposition_Action], '') = ISNULL(sla.[Sales_Line_Return_Deposition_Action], '')
			AND ISNULL(s.[Sales_Line_Return_Deposition_Description], '') = ISNULL(sla.[Sales_Line_Return_Deposition_Description], '')
		)
		/*
		UPDATE s
		SET Sales_Header_Property_Key_New = src.Sales_Header_Property_Key
		FROM [CWC_DW].[Fact].[Sales_Remapping] (ROWLOCK) s
		INNER JOIN src ON src.Sales_Order_Number = s.Sales_Order_Number AND src.Sales_Line_Number = s.Sales_Line_Number
		WHERE src.Sales_Header_Property_Key <> ISNULL(s.Sales_Header_Property_Key_New,'');
		*/
		--INSERT INTO Fact.Sales_Remapping
		--SELECT Sales_Key,Sales_Order_Number,Sales_Line_Number,Sales_Header_Property_Key,Sales_Header_Property_Key_NEW
		--FROM [CWC_DW].[Fact].[Sales]  
		,tgt
	AS (
		SELECT *
		FROM [CWC_DW].[Fact].[Sales]
	...
	WHEN NOT MATCHED
		THEN
			...
			VALUES (
				...
				,src.[Sales_Line_Number]
				,src.[Invent_Trans_ID]
				,src.[Return_Invent_Trans_ID]
				,src.[Sales_Header_Customer_Key]
				,src.[Sales_Header_Property_Key]
				,src.[Sales_Line_Property_Key]
				,src.[Sales_Header_Location_Key]
				,src.[Sales_Line_Product_Key]
				,src.[Shipping_Inventory_Warehouse_Key]
				,src.[Sales_Line_Employee_Key]
				,src.[Sales_Line_Amount]
				,src.[Sales_Line_Discount_Amount]
				,src.[Sales_Line_Cost_Price]
				,src.[Sales_Line_Discount_Percentage]
				,src.[Multi_Line_Discount_Code]
				,src.[Multi_Line_Discount_Percentage]
				,src.[Sales_Line_Reserve_Quantity]
				,src.[Sales_Line_Ordered_Quantity]
				,src.[Sales_Line_Price_Quantity]
				,src.[Sales_Line_Remain_Sales_Physical]
				,src.[Sales_Header_Modified_Date]
				,src.[Sales_Header_Created_Date]
				,src.[Sales_Line_Modified_Date]
				,src.[Sales_Line_Created_Date]
				,src.[Sales_Line_Confirmed_Date]
				,src.[Sales_Line_Confirmed_Receipt_Date]
				,src.[Sales_Line_Requested_Receipt_Date]
				,src.[Sales_Line_Created_By]
				,src.[Sales_Line_Price]
				,src.[Sales_Line_Ordered_Sales_Quantity]
	...
	WHEN MATCHED
		AND ISNULL(tgt.[Invent_Trans_ID], '') <> ISNULL(src.[Invent_Trans_ID], '')
		...
		THEN
			UPDATE
			SET [Invent_Trans_ID] = src.[Invent_Trans_ID]
	...
	UPDATE s
	SET Sales_Line_Property_Key = slp2.Sales_Line_Property_Key
	...
	LEFT JOIN [CWC_DW].[Dimension].[Sales_Line_Properties] slp2 ON CASE 
			WHEN s.is_Removed_From_Source = 1
				THEN 'Canceled'
			ELSE slp.Sales_Line_Status
			END = ISNULL(slp2.Sales_Line_Status, '')
	...
	WHERE s.is_Removed_From_Source = 1
		AND (slp2.Sales_Line_Property_Key <> slp.Sales_Line_Property_Key);
