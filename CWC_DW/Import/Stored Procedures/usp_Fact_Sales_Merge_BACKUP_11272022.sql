...
;WITH Sales_Order_Header AS
	(
		SELECT DISTINCT
			  ...
			  --,[Sales_Header_Created_Date_Key] = dc.Date_Key
			  --,[Sales_Header_Modified_Date_Key] = dm.Date_Key
			  ,[Sales_Header_Customer_Key] = c.Customer_Key 
			  ,[Sales_Header_Location_Key] = l.Location_Key
			  --,ISNULL(CAST(NULLIF(so.[DELIVERYADDRESSSTREET],'') AS VARCHAR(200)),'')
			  --,ISNULL(CAST(NULLIF(so.[DELIVERYADDRESSSTATEID],'') AS VARCHAR(5)),'')
			  --,ISNULL(CAST(NULLIF(so.[DELIVERYADDRESSCITY],'') AS VARCHAR(100)),'')
			  --,ISNULL(CAST(NULLIF(so.[DELIVERYADDRESSCOUNTRYREGIONID],'') AS VARCHAR(10)),'')
			  --,so.[ORDERINGCUSTOMERACCOUNTNUMBER]
			  ...
			  ,[Sales_Header_Created_Date] = CASE WHEN SALESCREATEDDATETIME='1900-01-01 00:00:00.000' THEN 
		...
		END--CAST(so.[SALESTABLECREATEDDATETIME] AS DATETIME2(5))
		  ...
		  LEFT JOIN [CWC_DW].[Dimension].[Customers] c (NOLOCK) ON c.Customer_Account = so.[ORDERINGCUSTOMERACCOUNTNUMBER]
		  --LEFT JOIN [CWC_DW].[Dimension].[Dates] dc (NOLOCK) ON dc.Calendar_Date = CAST(so.[SALESTABLECREATEDDATETIME] AS DATE)
		  --LEFT JOIN [CWC_DW].[Dimension].[Dates] dm (NOLOCK) ON dm.Calendar_Date = CAST(so.[SALESTABLEMODIFIEDDATETIME] AS DATE)
...
(		SELECT
			  ...
			  --,s.[Sales_Header_Created_Date_Key]
			  --,s.[Sales_Header_Modified_Date_Key]
			  --,[Sales_Line_Created_Date_Key] = dc.Date_Key
			  --,[Sales_Line_Modified_Date_Key] = dm.Date_Key
		...
		LEFT JOIN [CWC_DW].[Dimension].[Sales_Header_Properties] (NOLOCK) sha ON 
		...
		INNER JOIN [AX_PRODUCTION].[dbo].[ITSSalesOrderLineStaging] sl (NOLOCK) ON s.[Sales_Order_Number] = sl.[SALESORDERNUMBER]
		INNER JOIN [CWC_DW].[Dimension].[Products] (NOLOCK) p ON ISNULL(sl.ITEMNUMBER,'') = ISNULL(p.Item_ID,'') 
															 AND ISNULL(sl.PRODUCTCOLORID,'') = ISNULL(p.Color_ID,'') 
															 AND ISNULL(sl.PRODUCTSIZEID,'') = ISNULL(p.Size_ID,'')
		LEFT JOIN [CWC_DW].[Dimension].[Inventory_Warehouses] w (NOLOCK) ON w.Inventory_Warehouse_ID = sl.[SHIPPINGWAREHOUSEID]
		LEFT JOIN [CWC_DW].[Analytics].[v_Dim_Employees_By_User_ID] e (NOLOCK) ON e.[User_ID] = sl.SALESLINECREATEDBY
		--LEFT JOIN [CWC_DW].[Dimension].[Dates] dc (NOLOCK) ON dc.Calendar_Date = CAST(sl.[SALESLINECREATEDDATETIME] AS DATE)
		--LEFT JOIN [CWC_DW].[Dimension].[Dates] dm (NOLOCK) ON dm.Calendar_Date = CAST(sl.[SALESLINEMODIFIEDDATETIME] AS DATE)
		--LEFT JOIN [AX_PRODUCTION].[dbo].[SystemUserStaging] e (NOLOCK) ON e.UserID = sl.SALESLINECREATEDBY
		
) --SELECT * FROM Sales_Order_Header_Lines WHERE Sales_Order_Number = 'D00507253'
,src AS 
(
SELECT
	 [Sales_Order_Number]
	...
	--,[Sales_Header_Created_Date_Key]
	--,[Sales_Header_Modified_Date_Key]
	--,[Sales_Line_Created_Date_Key]
	--,[Sales_Line_Modified_Date_Key]
...
LEFT JOIN [CWC_DW].[Dimension].[Sales_Line_Properties] sla (NOLOCK) ON 
...
,tgt AS (SELECT * FROM [CWC_DW].[Fact].[Sales] (NOLOCK))
