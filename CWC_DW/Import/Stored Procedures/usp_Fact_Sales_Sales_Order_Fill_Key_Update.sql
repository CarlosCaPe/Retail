...
BEGIN TRY
	...
	AS (
		SELECT s.Sales_Key
			...
			,Fill_CD = CASE 
				WHEN ISNULL(SUM(s.[Sales_Line_Ordered_Quantity]), 0) <= SUM(i.Invoice_Line_Quantity)
					THEN 2 --Complete Fill
				WHEN ISNULL(SUM(s.[Sales_Line_Ordered_Quantity]), 0) > SUM(i.Invoice_Line_Quantity)
					THEN 1 --Partial Fill 
				ELSE 0
				END -- No Fill
	...
	AS (
		SELECT [Sales_Key] = s.[Sales_Key]
			...
			,[Days_To_Fill] = CASE 
				WHEN Fill_CD = 2
					AND [Order_Classification] = 'Sale'
					AND DATEDIFF(DAY, CAST([Sales_Line_Created_Date] AS DATE), i.[Invoice_Date]) > 0
					THEN DATEDIFF(DAY, CAST([Sales_Line_Created_Date] AS DATE), i.[Invoice_Date])
				WHEN Fill_CD = 2
					AND [Order_Classification] = 'Sale'
					THEN 0
				WHEN Fill_CD = 1
					AND [Order_Classification] = 'Sale'
					THEN DATEDIFF(DAY, CAST([Sales_Line_Created_Date] AS DATE), CAST(GETDATE() AS DATE))
				WHEN ISNULL(Fill_CD, 0) = 0
					THEN DATEDIFF(DAY, CAST([Sales_Line_Created_Date] AS DATE), CAST(GETDATE() AS DATE))
				END
		FROM [CWC_DW].[Fact].[Sales] s
	...
	LEFT JOIN [CWC_DW].[Dimension].[Sales_Order_Fill] sof ON Sales.Fill_CD = sof.Fill_CD
		AND Sales.Days_To_Fill BETWEEN sof.Range_Start
			AND ISNULL(sof.Range_Stop, Sales.Days_To_Fill)
