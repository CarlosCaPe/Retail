...
WITH src AS
	(
		SELECT DISTINCT
		  ...
		  --INTO CWC_DW.Fact.Warehouse_Shipping_Orders
		  FROM [AX_PRODUCTION].[dbo].[rsmWMSOrderPickingExportStaging](NOLOCK) p
		  INNER JOIN [CWC_DW].[Fact].[Sales] (NOLOCK) s ON s.Sales_Order_Number = p.[TRANSREFID] AND s.Invent_Trans_ID = p.inventTransId
			...
			/* SELECT DISTINCT
			  ...
			  ,[ETL_Modified_Count] = CAST(1 AS SMALLINT)
		  --INTO CWC_DW.Fact.Warehouse_Shipping_Orders
		  FROM [WMS_INV].[dbo].[EX940] (NOLOCK) p
		  INNER JOIN [CWC_DW].[Fact].[Sales] (NOLOCK) s ON s.Sales_Order_Number = p.SalesID AND s.Invent_Trans_ID = p.inventTransId */
	)
,tgt AS (SELECT * FROM [CWC_DW].[Fact].[Warehouse_Shipping_Orders] (NOLOCK))
