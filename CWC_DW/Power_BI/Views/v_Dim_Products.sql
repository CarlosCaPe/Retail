...
CREATE VIEW [Power_BI].[v_Dim_Products] AS
...
WITH Inventory AS
  ...
  ,[Available On Hand Quantity (Available)]
  FROM [CWC_DW].[Power_BI].[v_Fact_Inventory_Current]
	)
SELECT --TOP 1200
	  ...
	  ,[Item_ID Color_ID] = CONCAT(Item_ID,Color_ID)
      --,[Sales_Price]
      --,[Sales_Price_Date]
      --,[Purchase_Price]
      --,[Purchase_Price_Date]
	  ...
	  ,[Active Product] = p.Active_Product --CASE is_Active_Product WHEN 1 THEN 'Active Product' ELSE 'Not Active Product' END -- Create calculated field
	  ...
	  ,[Trade Agreement Price] = CASE WHEN p.Trade_Agreement_Price > 0 THEN p.Trade_Agreement_Price ELSE NULL END
	  ...
	  ,[Backordered Status] = p.Currently_Backordered
	  ,[Risk] = p.Risk
	  ...
	  ,[Hard Mark Price] = [Offer_Price_Hard_Mark]
	  ,[Soft Mark Price] = [Offer_Price_Soft_Mark]
	  ,[Timed Event Price] = [Offer_Price_Timed_Event]
	  ,[Current Price] = CASE 
		...
		,i.[Available On Hand Quantity (Available)]
		,i.[On Hand Quantity (Available)]
  ...
  FROM [CWC_DW].[Dimension].[Products] (NOLOCK) p
	  ...
	  LEFT JOIN Inventory i ON p.Product_Key = i.Product_Key
