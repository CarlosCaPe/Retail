...
CREATE VIEW [Power_BI].[v_Fact_Inventory_Snapshots]
...
SELECT [Snapshot Date Key] = [Snapshot_Date_Key]
  ...
  AND (d.Calendar_Date IN (SELECT TOP 20 DATEADD(DAY,1,Fiscal_Last_Day_of_Month) FROM [CWC_DW].[Dimension].[Dates] WHERE Calendar_Date <= CAST(GETDATE() AS DATE) AND DATEADD(DAY,1,Fiscal_Last_Day_of_Month) < CAST(GETDATE() AS DATE) GROUP BY  DATEADD(DAY,1,Fiscal_Last_Day_of_Month) ORDER BY DATEADD(DAY,1,Fiscal_Last_Day_of_Month) DESC)
OR is_Current_Inventory_Date = 1 
OR d.Calendar_Date IN ('03/01/2021','10/29/21','10/30/21','10/31/21','11/1/21','11/2/21','11/3/21')
)
;
