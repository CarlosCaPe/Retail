...
CREATE VIEW [Power_BI].[v_Fact_Sales_Sales]
...
SELECT --TOP 1000
  ...
  FROM [CWC_DW].[Fact].[Sales] (NOLOCK) s
	  INNER JOIN [CWC_DW].[Dimension].[Dates] (NOLOCK) d ON d.Calendar_Date = CAST(DATEADD(hh,-4,s.[Sales_Line_Created_Date]) AS DATE)
