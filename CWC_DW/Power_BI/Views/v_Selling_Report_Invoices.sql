CREATE VIEW [Power_BI].[v_Selling_Report_Invoices]
...
SELECT
	...
	,[Invoice Units] = i.Invoice_Line_Quantity
	...
	,[Sales Invoice Return Reason Group] = CAST((CASE WHEN slp.Sales_Line_Type = 'Return' THEN COALESCE(shp.[Sales_Header_Return_Reason_Group],'{-Not Populated-}') ELSE COALESCE(rshp.[Sales_Header_Return_Reason_Group],'{-Not Populated-}')  END) AS VARCHAR(100))
    ,[Sales Invoice Return Reason] = CAST((CASE WHEN slp.Sales_Line_Type = 'Return' THEN COALESCE(shp.[Sales_Header_Return_Reason],'{-Not Populated-}') ELSE COALESCE(rshp.[Sales_Header_Return_Reason],'{-Not Populated-}') END) AS VARCHAR(100))
	,[B2B Key] = CASE WHEN s.Sales_Header_Customer_Key IN (SELECT Customer_Key
				 ...
				 THEN 1 ELSE 2 END
	,[Customer Name] = c.Organization_Name
	,[Credit Only] = CASE WHEN p.[Item_ID] = 'CreditOnly' THEN 'Yes' ELSE 'No' END
FROM [CWC_DW].[Fact].[Invoices] (NOLOCK) i
	...
	LEFT JOIN [CWC_DW].[Fact].[Sales] rs ON s.Return_Invent_Trans_ID = rs.Invent_Trans_ID
	...
	LEFT JOIN [CWC_DW].[Dimension].[Sales_Header_Properties] (NOLOCK) rshp ON rs.Sales_Header_Property_Key = rshp.Sales_Header_Property_Key
	LEFT JOIN [CWC_DW].[Dimension].[Customers] (NOLOCK) c ON i.Invoice_Customer_Key = c.Customer_Key
WHERE 
